...
const createAnswersTemplate = (answer, author, commentsCount, answer_id, votes) => {
    let template =  `<div class="card">
                 ...
                 <input type="submit" id="${'commentButton'+answer_id}" value="coment" style='width:80px;'>
                 </span>
                 <hr>
 ...
 const createQestionTemplate = (question_id,title,body,creator, date) => {
    var template =    ` <div class="card">
                        <div class="container">
                        <h4 class='questionClass' question-id = '${question_id}'><b>${title}</b></h4>
                        <p>${body}</p> 
                        <a>Posted by<i> ${creator}</i></a>
                        <a id="countAnswer">${'On '+date}</a>
                        <a id='view'></a>
                        </div>
                        </div>`
    return template
...
const displayResult = () => {
    ...
    let date = response.question[0].asked_on;
    document.getElementById('quest').innerHTML = createQestionTemplate(question_id,title,body,author, date)
    let elements = '';
